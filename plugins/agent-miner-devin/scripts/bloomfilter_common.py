...
DEBUG_LOG_TAG = "devin"  # disambiguates plugins sharing the same log dir
...
DEFERRED_HOOK_EVENTS = frozenset()
...
TURN_TERMINAL_HOOK_EVENTS = frozenset({"Stop", "SessionEnd"})
...
def _resolve_debug_log_dir() -> str:
    ...
    %APPDATA%\\bloomfilter on Windows). Some runtimes inject a plugin-scoped data
    dir (Claude Code's CLAUDE_PLUGIN_DATA), but we deliberately ignore it so
