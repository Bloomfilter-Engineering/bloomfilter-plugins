...
def main():
    ...
    # On the web, session_id rotates on every resume but the transcript JSONL
    # preserves the original sessionId in its first entry. Use that as the
    # stable thread anchor for batching; fall back to session_id when no
    # transcript exists yet (e.g. SessionStart of a brand-new local session).
    ...
    # session_id is the per-resume ID (rotates on the web); thread_id is the
    # stable anchor recovered from the transcript.
    envelope = {
    ...
    # Append to batch file keyed by thread_id so resumes accumulate together
    ...
    if hook_event_name in UPLOAD_HOOKS:
        ...
        if not api_key:
            debug_log(
                ...
                f"thread_id={thread_id} reason=no-api-key"
            )
            return
        ...
        if not entries:
            debug_log(
                f"upload skipped: hook={hook_event_name} session_id={session_id} "
                f"thread_id={thread_id} reason=empty-batch"
            )
            return
