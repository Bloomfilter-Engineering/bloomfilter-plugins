...
# ---------------------------------------------------------------------------
# Transcript helpers (kept client-side — transcript is a local file)
# ---------------------------------------------------------------------------


def extract_thread_id(transcript_path):
    """Return the original sessionId from a transcript, or '' on failure.

    Claude Code on the web rotates session_id on each resume but preserves
    the original sessionId in the transcript JSONL entries. Reading the
    first entry recovers the stable thread anchor across resumes so we can
    batch them as a single conversation rather than treating each resume
